...
fun URI.maybeRedirect(): URI {
    ...
    val replaceWithCacheEntry = cacheMap.entries.find { (base, _) -> dealiasedUrl.startsWith(base) }
    return if (replaceWithCacheEntry != null) {
        URI("${replaceWithCacheEntry.value}${dealiasedUrl.substringAfter(replaceWithCacheEntry.key, "")}")
    } else {
        this
    }
}
