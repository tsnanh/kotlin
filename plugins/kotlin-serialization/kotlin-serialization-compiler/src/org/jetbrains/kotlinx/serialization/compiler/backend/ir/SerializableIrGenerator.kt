...
class SerializableIrGenerator(
        ...
        irClass.contributeConstructor(constructorDescriptor) { ctor ->
            ...
            if (useFieldMissingOptimization() &&
                ...
                val getDescriptorExpr = if (serializableDescriptor.isStaticSerializable) {
                    ...
                    // internal constructor creates only for internal classes - so companion exists
    ...
    private fun IrBlockBodyBuilder.getStaticSerialDescriptorExpr(): IrExpression {
        ...
        val serialDescriptorGetter = compilerContext.referenceClass(serializer.fqNameSafe)?.getPropertyGetter(SERIAL_DESC_FIELD)?:throw Exception("No class with name ${serializer.fqNameSafe}")
        return irGet(
