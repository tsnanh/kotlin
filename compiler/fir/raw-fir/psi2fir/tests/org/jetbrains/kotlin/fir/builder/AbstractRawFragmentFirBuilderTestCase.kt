...
abstract class AbstractRawFragmentFirBuilderTestCase : KtParsingTestCase(
    ...
    fun doRawFirTest(filePath: String) {
        ...
        val expected = InTextDirectivesUtils.findLinesWithPrefixesRemoved(text, "FIR_FRAGMENT_EXPECTED").toMutableList()

        val lineSet = LineSet.createLineSet(text)

        val notFoundActual = mutableListOf<String>()
        ...
        for (expression in testSeq.distinct()) {
            ...
            val line = lineSet.findLineIndex(expression.textOffset) + 1
            val actualLine = "LINE: $line TEXT OFFSET: ${expression.textOffset} LINE TEXT: ${expression.text.lines().first()}"

            if (!expected.remove(actualLine)) {
                notFoundActual.add(actualLine)
            }
        }

        if (expected.isNotEmpty()) {
            val linesNotFound = expected.joinToString(separator = "\n")
            TestCase.fail("Expected lines was not found in actual list\n$linesNotFound")
        }

        if (notFoundActual.isNotEmpty()) {
            val linesNotFound = notFoundActual.joinToString(separator = "\n")
            TestCase.fail("Actual lines was not found in expected list\n$linesNotFound")
        }
    }
