...
class AnnotationChecker(
    ...
    fun check(annotated: KtAnnotated, trace: BindingTrace, descriptor: DeclarationDescriptor? = null) {
        ...
        if (
            ...
            (annotated is KtCallableDeclaration || languageVersionSettings.supportsFeature(StrongRestrictionsForAnnotationsOnTypes))
        ) {
            annotated.typeParameters.forEach { check(it, trace) }
            for (typeParameter in annotated.typeParameters) {
                typeParameter.extendsBound?.let { checkTypeReference(it, trace, shouldCheckReferenceItself = true) }
            }
    ...
    private fun checkTypeReference(
        ...
        if (shouldRunCheck) {
            for (entry in reference.annotationEntries) {
                ...
                } else if (shouldRunCheck && languageVersionSettings.supportsFeature(StrongRestrictionsForAnnotationsOnTypes)) {
