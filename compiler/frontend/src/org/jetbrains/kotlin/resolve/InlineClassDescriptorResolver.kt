...
object InlineClassDescriptorResolver {
    ...
    private fun createConversionFunctionDescriptor(isBoxMethod: Boolean, owner: ClassDescriptor): SimpleFunctionDescriptor {
        ...
        val inlineClassType = owner.inlineClassRepresentation!!.underlyingType
        functionDescriptor.initialize(
            null,
            if (isBoxMethod) null else owner.thisAsReceiverParameter,
            emptyList<TypeParameterDescriptor>(),
            if (isBoxMethod) listOf(createValueParameterForBoxing(functionDescriptor, inlineClassType)) else emptyList(),
            if (isBoxMethod) owner.defaultType else inlineClassType,
            Modality.FINAL,
            DescriptorVisibilities.PUBLIC
        )
    ...
    private fun createValueParameterForBoxing(
        functionDescriptor: FunctionDescriptor, inlineClassType: KotlinType
    ): ValueParameterDescriptorImpl =
        createValueParameter(functionDescriptor, inlineClassType, BOXING_VALUE_PARAMETER_NAME, 0)

    private fun createValueParametersForSpecializedEquals(
        functionDescriptor: FunctionDescriptor, inlineClassType: KotlinType
    ): List<ValueParameterDescriptor> =
        listOf(
            createValueParameter(functionDescriptor, inlineClassType, SPECIALIZED_EQUALS_FIRST_PARAMETER_NAME, 0),
            createValueParameter(functionDescriptor, inlineClassType, SPECIALIZED_EQUALS_SECOND_PARAMETER_NAME, 1)
        )
